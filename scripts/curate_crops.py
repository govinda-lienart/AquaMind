...
def main(run_dir): 
    # 2 dataframes interacting
        # my_window -> from csv - 11 rows - one row per clean window containing range of frames with no swap
        # crops -> one row per crop image
    ...
    # accesing and listing the path of all the crops that were generated by the fish tracker
    banner("GATHER CROPS")
    crop_pattern = f"{run_dir}/crops/fish_*/*.jspg" # the * matches all files fish_1, fish_2,.... and same for file jpg...* matches all the images....frame001_fish1.jpg - inert string - nothing happens yet
    ...
    # testing: extract the frame number frome one crop filename (test) - used regex
    first_frame = crop_paths[0] # selecting the very first frame from the list of the path crops
    ...
    frame_num = int(m.group(1)) # note that m.group(0) is the whole match "frame_298_fish" while m.group(1) is what's inside the () e.g 298
    logger.info(f"for path {first_frame} the extracted frame number is {frame_num}")

    # generating a large dictionary that contains in it all refernece of each crop path, frame number and fish id
