...
def load_windows(run_dir):
    """load the verified-windows CSV and filter to this video's windows"""
    # loading the csv files containing the clean-no-swap-streches
    ...
    # selecting in the dataframe the relevant output using argument provided by user about video_output 
...
def build_crops_table(run_dir):
    """glob every crop path, parse frame_number + fish_id from filenames, return crops dataframe"""
    banner_sub("GATHER CROPS")
    ...
    banner_sub("BUILD CROPS DATAFRAME")
    rows = [] # list of dictionaries
    for path in crop_paths:
        m = re.search(r"frame_(\d+)_fish_(\d+)",path)
        if m is None:                                  # NEW guard (replaces the old test block)
...
def main(run_dir): 
    ...
    # interaction between 2 dataframes (windows x crops) using  range join - tagging the crops to which curated window they belong
