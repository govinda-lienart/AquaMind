"""usage:  python -m scripts.train_chase_lstm_windowed
Phase F (windowed variant) - LSTM trained on the SAME fixed 35-frame/17-stride windows
build_chase_windows.py (Phase D) generates for the sklearn baselines, so training input shape
matches what a deployment sliding-window scanner would actually feed it. Matched RF/XGBoost's
confusion matrix exactly (22/2/0/16) on the last comparison run - see diary.md, Stage 7."""
...
HIDDEN_SIZE = 16  # small on purpose - only ~90 training windows, a bigger LSTM would just overfit
NUM_EPOCHS = 60
LEARNING_RATE = 0.005
...
def main():
    ...
    banner('STEP 9 - EVALUATE ON TEST WINDOWS')
    model.eval()
    with torch.no_grad():
    ...
    output_folder = os.path.join(split_folder, 'output_train_chase_lstm_windowed', stamp)
    ...
    ax.set_title('lstm_windowed')
    path_plot = os.path.join(output_folder, 'lstm_windowed_confusion_matrix.png')
    ...
    run_config = {
        ...
        'window_size_frames': WINDOW_SIZE_FRAMES, 'stride_frames': STRIDE_FRAMES,
        'hidden_size': HIDDEN_SIZE, 'num_epochs': NUM_EPOCHS, 'learning_rate': LEARNING_RATE,
        'n_train_windows': len(train_sequences), 'n_test_windows': len(test_sequences),
    }
